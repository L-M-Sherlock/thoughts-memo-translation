...
这两个理念的实现都依赖于大语言模型（LLMs）的最新进展。不过，我们运用大语言模型的方式与大多数「提升生产力」的系统有着本质区别。那些系统通常致力于让大语言模型来*完成*任务。但实际上，大语言模型并不能真正「完成」我前面提到的那些闪念。正如 Taylor 巧妙地表述：其他系统是你在引导大语言模型，而我们的系统是*让大语言模型来引导你*。
