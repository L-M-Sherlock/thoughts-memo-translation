...
当使用 **Plan** 时，你会得到一个日程表，其中可以看到各活动的时间分配。然后 SuperMemo 会检查可用时间，如果可用时间不够，剩下未完成的各活动会按比例缩减。如果你每天的活动是重复性的，你可以在最后时刻对你的日程安排进行微调。如果那些活动不是重复性的，你仍然可以通过提前计划并在进行时跟踪你的进展而受益。

如果你不确定 **Plan** 对你来说是否是一个有用的工具，请阅读：「**[规划一个没有压力的完美生产日](https://supermemo.guru/wiki/Planning_a_perfect_productive_day_without_stress)**」。未来的社会一定会朝着不设截止日、有创造性工作的方向发展，这非常符合 **Plan** 提供的弹性日程安排。
